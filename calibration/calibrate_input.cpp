...
namespace
{
  ...
  bool find_corners(const cv::Mat & image, const cv::Size & pattern_size, std::vector<cv::Point2f> & corners, bool fast_check)
  {
    cv::Mat gray;
  ...
  std::vector<int> image_ids(const fs::path & folder)
  {
    ...
    for (const auto & entry : fs::directory_iterator(folder))
    {
      ...
      if (stem.empty() || !std::all_of(stem.begin(), stem.end(), [](unsigned char ch) { return std::isdigit(ch); }))
      {
        continue;
      }
  ...
  {
    const double scale = std::min({1.0, static_cast<double>(max_width) / image.cols, static_cast<double>(max_height) / image.rows});
  ...
  void draw_translucent_rect(cv::Mat & image, const cv::Rect & rect, const cv::Scalar & color, double opacity)
  {
  ...
  void draw_label(cv::Mat & image, const std::string & text, cv::Point origin, double scale, const cv::Scalar & color, int thickness = 1)
  {
    cv::putText(image, text, origin, cv::FONT_HERSHEY_SIMPLEX, scale, cv::Scalar(0, 0, 0), thickness + 2, cv::LINE_AA);
    cv::putText(image, text, origin, cv::FONT_HERSHEY_SIMPLEX, scale, color, thickness, cv::LINE_AA);
  }

  cv::Mat make_capture_view(
    const cv::Mat & image, const CalibrationConfig & config, const std::vector<cv::Point2f> & corners, bool board_found, const Eigen::Vector3d & ypr,
    int saved_count)
  {
    ...
    draw_translucent_rect(view, cv::Rect(0, view.rows - bottom_height, view.cols, bottom_height), cv::Scalar(20, 24, 28), 0.88);
    ...
    const auto pose_text = fmt::format("YAW {:+.1f}   PITCH {:+.1f}   ROLL {:+.1f}", ypr[0], ypr[1], ypr[2]);
    int pose_baseline = 0;
    cv::Size pose_size = cv::getTextSize(pose_text, cv::FONT_HERSHEY_SIMPLEX, 0.52, 1, &pose_baseline);
    const std::string board_text = view.cols >= 620 ? (board_found ? "BOARD DETECTED" : "BOARD NOT FOUND") : (board_found ? "BOARD OK" : "NO BOARD");
    ...
    if (view.cols >= 620)
    {
      ...
      const auto saved_size = cv::getTextSize(saved_text, cv::FONT_HERSHEY_SIMPLEX, 0.58, 2, &saved_baseline);
  ...
  void show_review(const cv::Mat & image, const CalibrationConfig & config, const std::vector<cv::Point2f> & corners, bool found, int current, int total)
  {
    cv::Mat view = image.clone();
    ...
    draw_label(view, found ? "CHECKING BOARD   PASS" : "CHECKING BOARD   SKIP", {22, bar_height / 2 + 8}, 0.64, color, 2);
    ...
    draw_label(view, progress, {view.cols - size.width - 22, bar_height / 2 + 8}, 0.6, cv::Scalar(238, 242, 245), 2);
    cv::imshow(kWindowName, view);
    cv::waitKey(100);
  }

  std::vector<Sample> load_samples(const fs::path & folder, const CalibrationConfig & config, cv::Size & image_size)
  {
    const auto ids = image_ids(folder);
  ...
  CameraCalibration calibrate_camera(const std::vector<Sample> & samples, const CalibrationConfig & config, const cv::Size & image_size)
  {
    ...
    const auto criteria = cv::TermCriteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS, 100, DBL_EPSILON);
    cv::calibrateCamera(object_points, image_points, image_size, result.camera_matrix, result.distort_coeffs, rvecs, tvecs, 0, criteria);
    ...
    for (size_t i = 0; i < object_points.size(); ++i)
    {
      ...
      cv::projectPoints(object_points[i], rvecs[i], tvecs[i], result.camera_matrix, result.distort_coeffs, reprojected_points);
  ...
  HandEyeCalibration calibrate_handeye(const std::vector<Sample> & samples, const CalibrationConfig & config, const CameraCalibration & camera)
  {
    ...
    {
      cv::Mat rvec, tvec;
      if (!cv::solvePnP(board_points, sample.corners, camera.camera_matrix, camera.distort_coeffs, rvec, tvec, false, cv::SOLVEPNP_ITERATIVE))
      {
      ...
      const Eigen::Matrix3d R_gimbal2world = config.R_gimbal2imubody.transpose() * R_imubody2imuabs * config.R_gimbal2imubody;
    ...
    if (rvecs.size() < 3)
    {
      throw std::runtime_error(fmt::format("手眼标定至少需要 3 组有效样本，当前只有 {} 组", rvecs.size()));
    }

    cv::Mat R_gimbal2camera, t_gimbal2camera;
    cv::Mat R_world2board, t_world2board;
    cv::calibrateRobotWorldHandEye(rvecs, tvecs, R_world2gimbal_list, t_world2gimbal_list, R_world2board, t_world2board, R_gimbal2camera, t_gimbal2camera);
    t_gimbal2camera /= 1e3;
    t_world2board /= 1e3;
  ...
  {
    YAML::Emitter result;
    const std::vector<double> camera_matrix_data(camera.camera_matrix.begin<double>(), camera.camera_matrix.end<double>());
    const std::vector<double> distort_coeffs_data(camera.distort_coeffs.begin<double>(), camera.distort_coeffs.end<double>());
  ...
  {
    YAML::Emitter result;
    const std::vector<double> R_camera2gimbal_data(handeye.R_camera2gimbal.begin<double>(), handeye.R_camera2gimbal.end<double>());
    const std::vector<double> t_camera2gimbal_data(handeye.t_camera2gimbal.begin<double>(), handeye.t_camera2gimbal.end<double>());
    ...
    result << YAML::Newline;
    result << YAML::Comment(
      fmt::format("相机同理想情况的偏角: yaw{:.2f} pitch{:.2f} roll{:.2f} degree", handeye.camera_ypr[0], handeye.camera_ypr[1], handeye.camera_ypr[2]));
    result << YAML::Newline;
    result << YAML::Comment(fmt::format("标定板到世界坐标系原点的水平距离: {:.2f} m", handeye.board_distance));
    result << YAML::Newline;
    result << YAML::Comment(
      fmt::format("标定板同竖直摆放时的偏角: yaw{:.2f} pitch{:.2f} roll{:.2f} degree", handeye.board_ypr[0], handeye.board_ypr[1], handeye.board_ypr[2]));
  ...
  {
    if (line.empty() || std::isspace(static_cast<unsigned char>(line.front())) || line.front() == '#')
    {
      return -1;
    }
  ...
  void update_calibration_yaml(const std::string & config_path, const CameraCalibration & camera, const HandEyeCalibration & handeye)
  {
    const std::vector<double> camera_matrix_data(camera.camera_matrix.begin<double>(), camera.camera_matrix.end<double>());
    const std::vector<double> distort_coeffs_data(camera.distort_coeffs.begin<double>(), camera.distort_coeffs.end<double>());
    const std::vector<double> R_camera2gimbal_data(handeye.R_camera2gimbal.begin<double>(), handeye.R_camera2gimbal.end<double>());
    ...
    for (size_t i = 0; i < lines.size(); ++i)
    {
      ...
      {
        while (i + 1 < lines.size() && !lines[i + 1].empty() && std::isspace(static_cast<unsigned char>(lines[i + 1].front())))
        {
          ++i;
        }
      }
  ...
  bool run_calibration(const fs::path & folder, const CalibrationConfig & config, const std::string & config_path)
  {
  ...
  void capture_loop(const std::string & config_path, const fs::path & input_folder, const CalibrationConfig & config)
  {
    ...
    while (true)
    {
      ...
      const Eigen::Matrix3d R_gimbal2world = config.R_gimbal2imubody.transpose() * q.toRotationMatrix() * config.R_gimbal2imubody;
