...
void NumberClassifier::extractNumbers(const cv::Mat & src, std::vector<Armor> & armors)
{
  ...
  for (auto & armor : armors) {
    ...
    cv::Point2f lights_vertices[4] = {
      armor.left.bottom, armor.left.top, armor.right.top,
      armor.right.bottom};
...
void NumberClassifier::classify(std::vector<Armor> & armors)
{
  for (auto & armor : armors) {
    ...
    if (label_str == "1") armor.name = ArmorName::one;
    else if (label_str == "2") armor.name = ArmorName::two;
    else if (label_str == "3") armor.name = ArmorName::three;
    else if (label_str == "4") armor.name = ArmorName::four;
    else if (label_str == "5") armor.name = ArmorName::five;
    else if (label_str == "sentry" || label_str == "guard") armor.name = ArmorName::sentry;
    else if (label_str == "outpost") armor.name = ArmorName::outpost;
    else if (label_str == "base") armor.name = ArmorName::base;
    else armor.name = ArmorName::not_armor;
    // ---------------------------------

    // 依然可以保留 string 形式用于调试打印
    std::stringstream result_ss;
    result_ss << label_str << ": " << std::fixed << std::setprecision(1)
  ...
  armors.erase(
    std::remove_if(
      ...
      [this](const Armor & armor) {
        ...
        if (armor.name == ArmorName::one) current_label = "1";
        else if (armor.name == ArmorName::two) current_label = "2";
        else if (armor.name == ArmorName::three) current_label = "3";
        else if (armor.name == ArmorName::four) current_label = "4";
        else if (armor.name == ArmorName::five) current_label = "5";
        else if (armor.name == ArmorName::sentry) current_label = "sentry"; // 注意这里可能是 sentry 或 guard
        else if (armor.name == ArmorName::outpost) current_label = "outpost";
        else if (armor.name == ArmorName::base) current_label = "base";
        else current_label = "not_armor";
