...
namespace auto_aim
{
  ...
  bool Shooter::shoot(
    const io::Command & command, const auto_aim::Aimer & aimer, const std::list<auto_aim::Target> & targets, const Eigen::Vector3d & gimbal_pos)
  {
    ...
    auto tolerance = std::sqrt(tools::square(target_x) + tools::square(target_y)) > judge_distance_ ? second_tolerance_ : first_tolerance_;
    ...
    const io::sb_VisionToGimbal & vision_cmd, const auto_aim::Aimer & aimer, const std::list<auto_aim::Target> & targets, const Eigen::Vector3d & gimbal_pos)
  {
    ...
    // tools::logger()->debug("d(yaw) is {:.4f}", std::abs(last_yaw_ - vision_cmd.yaw));
    if (
      std::abs(last_yaw_ - vision_cmd.yaw) < tolerance * 2 &&  // 此时认为yaw突变不应该射击
      std::abs(gimbal_pos[0] - last_yaw_) < tolerance &&       // 云台当前位置与上一次命令yaw的差值
      aimer.debug_aim_point.valid)
    {
      last_yaw_ = vision_cmd.yaw;
      return true;
    }
