...
namespace auto_aim
{
  ...
  void Voter::vote(const Color color, const ArmorName name, const ArmorType type) { count_[index(color, name, type)] += 1; }

  std::size_t Voter::count(const Color color, const ArmorName name, const ArmorType type) { return count_[index(color, name, type)]; }
